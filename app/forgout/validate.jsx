...
export default function Validate() {
  ...
  const { registrationData, code } = useAuthStore.getState();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (data.otp == code) {
        Alert.alert("Codigo correcto", "El código ingresado es correcto");
        reset();
        router.push("/forgout/change")
      } else {
  ...
  return (
    <View className="flex-1 justify-center items-center p-5">
      <Text className="text-lg font-semibold mb-4">Recuperacion de Cuenta</Text>
